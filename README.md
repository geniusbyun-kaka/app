...
- **배당 탭**: 종목 스냅샷에 함께 저장되는 배당 이력(야후, 배당락일 기준 주당 금액)으로 배당 캘린더를 만듭니다. 시가배당률(최근 12개월 실제 지급 ÷ 현재가), 연 배당 예상(정기 지급액 중앙값 × 연 횟수), 세전·세후(기본 미국 원천징수 15%, 조정 가능), 이번 주·이번 달 배당락 목록, 보유 수량 입력 시 연간·이번 달 예상 수령액(달러·원화), 종목 간 배당 비교(연도별 주당 배당, 5년 성장률). 다음 배당락일은 최근 지급 간격으로 계산한 예상일이며 지급일은 표시하지 않습니다. 배당 ETF 16개(VYM, VIG, DGRO, HDV, SPYD, DVY, SDY, NOBL, DIVO, QYLD, XYLD, RYLD, SPHD, TLTW, MAIN, ARCC)를 종목 목록에 추가했습니다.
